...
/** Efficiently manages mutex checking for options */
struct _ap_Mutex_manager {
	/** Backpointer to parser */
	const struct _arg_parser *parser;
	/** Contains for each known option an array of mutex identifiers */
	int **option_to_mutex;
	/** Contains for each known option the number of mutex defined for it */
	int *option_to_mutex_size;
...
/**
 * Initialize mutex manager based on an already initialized parser and a definition
 * of mutual exclusions.
 *
 * @param[in] ap Pointer to arg parser state
 * @param[in] mm Pointer to mutex manager to initialize
 * @param[in] mutex_definition Defines mutual exclusions for options specified in \p ap .
 * It contains option codes for each mutex separated by '0' and terminated by '00'. Example:
 * {2,3,0,5,6,0,0} describes two mutex {2,3} and {5,6}.
 ...
 *
 * @param[in] mm Pointer to an initialized mutex manager
