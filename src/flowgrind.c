...
static void parse_host_option(const char* hostarg, int flow_id, int endpoint_id)
{
	...
	if (extra_rpc) {
		...
		if (port < 1 || port > 65535)
			PARSE_ERR("flow %i: invalid port for RPC",
					flow_id);
	}
							...
							int flow_id)
{
	...
	case 'J':
		if (sscanf(arg, "%u", &optunsigned) != 1)
			PARSE_ERR("option %s needs an integer argument", opt_string);
...
static void parse_cmdline(int argc, char *argv[])
{
	...
	struct _ap_Mutex_manager mm;
  	if (!ap_init_mutex_manager(&parser, &mm, mutex))
		critx("could not allocate memory for mutex manager");
	
	/* initialize 3 mutex contexts (for SOURCE+DESTINATION+NO_ENDPOINT) */
  	struct _ap_Mutex_state ms[3];
	ap_init_mutex_state(&mm, &ms[SOURCE]);
	ap_init_mutex_state(&mm, &ms[DESTINATION]);
	ap_init_mutex_state(&mm, &ms[NO_ENDPOINT]);
	...
	for (int argind = 0; argind < ap_arguments(&parser); argind++) {
		...
		switch (tag) {
		case OPT_CONTROLLER:
			check_mutex(&mm, ms, NO_ENDPOINT, argind, 0);
		...
		case OPT_SELECTOR:
			...
			break;
		case OPT_FLOW:
			check_mutex(&mm, ms, SOURCE, argind, current_flow_ids[0]);
		...
		case OPT_FLOW_ENDPOINT:
			...
			for (char *token = strtok(argcpy, ","); token;
				...
				if (type == 's' || type == 'b')
					check_mutex(&mm, ms, SOURCE, argind, 
						current_flow_ids[0]);
				if (type == 'd' || type == 'b')
					check_mutex(&mm, ms, DESTINATION, argind, 
						current_flow_ids[0]);		
